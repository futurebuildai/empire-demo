...
import type { Statement, PaymentTransaction } from '../../connect/types/domain.js';
...
export class PvPageBilling extends PvBase {
  ...
  @state() private lastPayment: PaymentTransaction | null = null;
  ...
  private async loadInvoices(initialLoad = false) {
    try {
      ...
      const response = await SalesService.getInvoices(this.invoicesPageSize, offset, jobId);

      // Map to legacy format
      const { mapInvoiceToLegacy } = await import('../../connect/mappers.js');
      this.invoices = (response.items as any[]).map(mapInvoiceToLegacy);
  ...
  private async loadInvoiceSummary() {
    try {
      ...
      const { mapInvoiceToLegacy } = await import('../../connect/mappers.js');
      const allInvoices = (response.items as any[]).map(mapInvoiceToLegacy);
      const outstanding = allInvoices.filter((i: Invoice) => i.status === 'open' || i.status === 'past_due');
  ...
  private async loadLastPayment() {
    try {
      const history = await BillingService.getPaymentHistory();
      // Filter valid successful (or processing) payments
      // Exclude failed, cancelled, voided. Refunded is debatable but usually not "Last Payment" in positive sense.
      const valid = history.filter(p => !['failed', 'cancelled', 'voided', 'refunded'].includes(p.status));
      // Sort by submittedAt descending
      valid.sort((a, b) => new Date(b.submittedAt).getTime() - new Date(a.submittedAt).getTime());

      if (valid.length > 0) {
        this.lastPayment = valid[0];
      }
    } catch (e) {
  ...
  private renderListView() {
    ...
    return html`
      <div class="billing-summary">
        ...
        <div class="summary-card">
          ...
          <div class="summary-value">
            ${this.lastPayment ? this.formatCurrency(this.lastPayment.totalCharged) : '—'}
          </div>
          ${this.lastPayment ? html`
            <div style="font-size: var(--text-xs); color: var(--color-text-muted); margin-top: 4px;">
              ${this.formatDate(this.lastPayment.submittedAt)}
            </div>
          ` : ''}
        </div>
      </div>
      ...
      <div class="billing-tabs">
        <button class="billing-tab ${this.activeTab === 'invoices' ? 'active' : ''}" @click=${() => this.activeTab = 'invoices'}>Invoices</button>
