...
export class PageAccountDetails extends LitElement {
    ...
    async connectedCallback() {
        ...
        if (id) {
            try {
                ...
                // Sync financials with actual invoices
                const totalBalance = this.allOpenInvoices.reduce((sum, inv) => sum + inv.balance, 0);
                const pastDueBalance = this.allOpenInvoices
                ...
                this.account = {
                    ...
                    availableCredit: account.creditLimit - totalBalance
                } as any;
    ...
    @state() private selectedMethod: 'email' | 'text' = 'text';
    @state() private selectedContact = '';
                    ...
                    <table class="data-table">
                        ...
                        openInvoices.map(i => html`
                                    <tr class="${this.selectedInvoices.has(i.id) ? 'row-selected' : ''}">
                                        ...
                                        <td>${this.formatDate(i.dueDate)}</td>
                ...
                return html`
                    <table class="data-table">
                        ...
                        openQuotes.map(q => html`
                                    <tr>
                                        <td class="font-medium">
                                            <a href="/admin/estimates/${q.id}" style="color: var(--color-primary, #2563eb); text-decoration: none; font-weight: 500;">
                                                ${q.name}
                                            </a>
                                        </td>
    ...
    private openMessageModal() {
        ...
        this.selectedContact = this.account?.phone || '';
    ...
    private async handleSendMessage() {
        ...
        try {
            const thread = await AdminDataService.createThread(
                this.account.id,
                this.selectedMethod,
                [this.selectedContact]
            );
            this.closeMessageModal();
            Router.go(`/admin/messaging?threadId=${thread.id}`);
        } catch (e) {
            console.error(e);
            ...
            ${this.showMessageModal ? html`
                <div class="modal-overlay" @click=${this.closeMessageModal} role="dialog" aria-modal="true" aria-label="Send message">
                    <div class="modal" @click=${(e: Event) => e.stopPropagation()}>
                        <div class="modal-title">Start Conversation</div>
                            ...
                            <div style="display: flex; gap: 1rem; margin-top: 0.5rem;">
                                <label style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer;">
                                    <input type="radio" name="method" value="text" 
                                        .checked=${this.selectedMethod === 'text'} 
                                        @change=${() => { this.selectedMethod = 'text'; this.selectedContact = a.phone; }}
                                    >
                                    Text Message
                                </label>
                                <label style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer;">
                                    <input type="radio" name="method" value="email" 
                                        .checked=${this.selectedMethod === 'email'}
                                        @change=${() => { this.selectedMethod = 'email'; this.selectedContact = a.email; }}
                                    >
                                    Email
                                </label>
                            </div>
                        ...
                        <div style="margin-bottom: 1.5rem;">
                            <label class="label">Recipient (${this.selectedMethod === 'text' ? 'Phone' : 'Email'})</label>
                            ${this.selectedMethod === 'text' ? html`
                                <select class="form-select" .value=${this.selectedContact} @change=${(e: Event) => this.selectedContact = (e.target as HTMLSelectElement).value}>
                                    <option value="${a.phone}">${a.primaryContact}: ${a.phone}</option>
                                    <!-- Potentially add other contacts if available in the future -->
                                </select>
                            ` : html`
                                <select class="form-select" .value=${this.selectedContact} @change=${(e: Event) => this.selectedContact = (e.target as HTMLSelectElement).value}>
                                    <option value="${a.email}">${a.primaryContact}: ${a.email}</option>
                                </select>
                            `}
                        </div>
                        ...
                        <div class="modal-actions">
                            ...
                            <button class="btn-primary" @click=${this.handleSendMessage}>Start Thread</button>
                        </div>
                    </div>
                </div>
            ` : ''}
