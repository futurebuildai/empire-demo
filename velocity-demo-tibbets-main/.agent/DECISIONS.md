...
## DEC-022: Contextual Search Pattern

**Date:** 2026-02-11
**Status:** Decided

### Context
Global header search was competing with the primary Hero search on the landing page and felt redundant in the Pro portal context.

### Decision
1. **Removed**: Global product search bar from the header (`pv-header.ts`).
2. **Integrated**: Added catalog search functionality directly into the **Quote Drawer** and **Cart Drawer**.
3. **Primary Entry**: Search remains the central focus of the **Hero Section** on the landing page.

### Rationale
- Improves UI cleanliness in the header.
- Provides search where it is most actionable (while building a quote or checking out).
- Aligns with "task-oriented" Pro user workflow.

---

## DEC-023: Drawer Focus & Collision Prevention

**Date:** 2026-02-11
**Status:** Decided

### Context
Overlapping drawers (Quote and Cart) and their absolute positioning led to pointer-event collisions where a hidden drawer could block clicks to the header.

### Decision
1. **Pointer Event Management**: Set `pointer-events: none` on drawer containers by default, enabling it (`pointer-events: auto`) only when the `[open]` attribute is present.
2. **State Consolidation**: Removed duplicate closing handlers in `pv-app.ts` to ensure consistent state transitions.
